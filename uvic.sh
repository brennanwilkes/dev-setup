...
echo 'ec2() { ssh ubuntu@`~/aws-cli/v2/current/bin/aws ec2 describe-instances --filters Name=instance-id,Values=i-0fb1b5499918be07f --output text | grep -oE "ec2-[0-9-]*.us-west-.*.compute.amazonaws.com" | head -n1` ; }' | addToFile
